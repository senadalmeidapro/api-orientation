...
export class EmailService implements OnModuleInit {
  ...
  private resolveSubject(explicit: string, templateSubject: string, templateName: string): string {
  ...
  async sendPasswordResetEmail(recipient: MailRecipient, resetLink: string, expiresIn = '1 heure') {
    return this.sendEmail({
      to: recipient.email,
      subject: recipient.subject,
