...
export class AuthTokenService {
  ...
  async signTokens(user: JwtUser): Promise<SignedTokens> {
    ...
    const accessToken = await this.jwt.signAsync(accessPayload, this.getJwtSignOptions('access'));
  ...
  private getJwtSignOptions(tokenType: 'access' | 'refresh'): JwtSignOptions {
    return {
      ...
      expiresIn:
        tokenType === 'access' ? this.config.jwt.accessExpiresIn : this.config.jwt.refreshExpiresIn,
      issuer: this.config.jwt.issuer,
      audience: this.config.jwt.audience,
    };
  }
