...
export class AuthService {
  ...
  async register(dto: RegisterDto, req?: Request) {
    ...
    if (user.email) {
      await this.email.sendVerificationEmail(
        ...
        `${this.config.app.front}/auth/check-email?token=${verificationToken}`,
      );
    }
  ...
  async passwordResetRequest(dto: EmailDto, req?: Request) {
    ...
    if (!user || user.isDeleted || user.status === UserStatus.DELETED) {
      return {
        message: 'Si un compte existe pour cet email, un message de réinitialisation a été envoyé.',
      };
    }
    ...
    if (user.email) {
      await this.email.sendPasswordResetEmail(
        ...
        `${this.config.app.front}/auth/reset-password?token=${token}`,
      );
    }
  ...
  async validateUserFromJwt(payload: JwtPayload) {
    const issuer =
      payload.issuer ?? (payload as JwtPayload & { iss?: string; aud?: string | string[] }).iss;
    if (!issuer || issuer !== this.config.jwt.issuer) {
      throw new UnauthorizedException('Invalid token');
    }

    const audience =
      payload.audience ?? (payload as JwtPayload & { iss?: string; aud?: string | string[] }).aud;
