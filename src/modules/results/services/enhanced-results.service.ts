...
export class EnhancedResultsService {
  ...
  async formatBehavioralObservations(assessmentId: string): Promise<string[]> {
    ...
    if (metrics.averageResponseTime > 0) {
      formatted.push(`Temps moyen de réponse: ${(metrics.averageResponseTime / 1000).toFixed(1)}s`);
    }
  ...
  private formatRiasecSection(snapshot: ProfileSnapshot): string {
    return (
      ...
      snapshot.topThree
        .map((t) => `- ${t.type}: ${t.percentage.toFixed(1)}% (${t.score.toFixed(2)} points)`)
        .join('\n')
    );
  }
  ...
  private generateActionPlan(
    ...
    plan.push(`Explorez les carrières liées à votre profil dominant: ${profileSnapshot.dominant}`);
